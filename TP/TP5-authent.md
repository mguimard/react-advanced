...
Le but de ce TP est d'utiliser useContext pour gérer une authentification.
...
Ce serveur vous renvoie un token si vous faites un POST sur /login

## Modifications coté client

Ajouter un bouton "login" dans le menu de gauche. Lors d'un click sur le bouton, récupérer un token depuis le serveur.

Créer un contexte pour avoir accès à une variable "isAuthed" et une méthode setAuthToken.

Appeler setAuthToken lors de la récupération du token.

Utiliser isAuthed pour afficher ou cacher les controles des LEDs et employés.

## Bonus : Stockage du token
